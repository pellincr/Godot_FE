...
func update_by_unit():
	weapon_icon_container.unit = unit.unit
	weapon_icon_container.set_icon_visibility_unit()
	unit_experience_container.unit = unit.unit
	unit_experience_container.update_by_unit()
	unit_stat_progression.unit = unit
	unit_stat_progression.update_by_unit()
	set_unit_name_label(unit.unit.name)
	set_hp_value(unit.current_hp)
	set_hp_bar(unit.current_hp)
	unit_inventory_container.unit = unit.unit
	unit_inventory_container.update_by_unit()
	update_unit_icon(unit.unit.icon)
