...
func update_by_unit():
	...
	unit_type = UnitTypeDatabase.get_definition(unit.unit.unit_type_key)

	set_label_with_number(strength_value_label,unit.get_strength())
	set_label_with_number(strength_growth_label,unit.unit.growths.strength, true)
	set_progress_bar(strength_progress,unit.get_strength(), unit_type.maxuimum_stats.strength)
	
	set_label_with_number(magic_value_label,unit.get_magic())
	set_label_with_number(magic_growth_label,unit.unit.growths.magic, true)
	set_progress_bar(magic_progress,unit.get_magic(), unit_type.maxuimum_stats.magic)
	
	set_label_with_number(skill_value_label,unit.get_skill())
	set_label_with_number(skill_growth_label,unit.unit.growths.skill, true)
	set_progress_bar(skill_progress,unit.get_skill(), unit_type.maxuimum_stats.skill)
	
	set_label_with_number(speed_value_label,unit.get_speed())
	set_label_with_number(speed_growth_label,unit.unit.growths.speed, true)
	set_progress_bar(speed_progress,unit.get_speed(), unit_type.maxuimum_stats.speed)
	
	set_label_with_number(luck_value_label,unit.get_luck())
	set_label_with_number(luck_growth_label,unit.unit.growths.luck, true)
	set_progress_bar(luck_progress,unit.get_luck(), unit_type.maxuimum_stats.luck)
	
	set_label_with_number(defense_value_label,unit.get_defense())
	set_label_with_number(defense_growth_label,unit.unit.growths.defense, true)
	set_progress_bar(defense_progress,unit.get_defense(), unit_type.maxuimum_stats.defense)
	
	set_label_with_number(resistance_value_label,unit.get_resistance())
	set_label_with_number(resistance_growth_label,unit.unit.growths.resistance, true)
	set_progress_bar(resistance_progress,unit.get_resistance(), unit_type.maxuimum_stats.resistance)
	
	set_label_with_number(move_value_label,unit.unit.stats.movement)
	
	set_label_with_number(constituiton_value_label,unit.unit.stats.constitution)
