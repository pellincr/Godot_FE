...
func _process(delta):
	...
	if has_focus():
		if on_tab_view:
			...
			self.theme = preload("res://ui/battle_prep_new/shop/shop_container/tab_icon/big_and_bold.tres")
		else:
			#when not on tab view but has vocus should be big but not bold
			self.theme = preload("res://ui/battle_prep_new/shop/shop_container/tab_icon/not_big_but_bold.tres")
	else:
		#when does not have focus should not be big or bold
		self.theme = preload("res://ui/battle_prep_new/shop/shop_container/tab_icon/not_big_not_bold.tres")
