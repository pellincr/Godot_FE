...
##SIGNALS
signal tile_info_updated(tile : Dictionary)  #SHOULD THIS BE REMOVED?

##EXPORTS
@export var terrain_tile_map : TileMap #TO BE UPDATED TO TILE MAP LAYERS
...
#initialized the grid after setting the terrain
func initialize():
...
func get_entity(position:Vector2i)-> CombatMapEntity:
	...
	return null
##
# Update combat map game grid to reflect a units move
##
...
##
# Places a combat unit into the game grid at position
##
...
##
# Checks game grid to see if the current position occupied
##
...
##
# Checks game grid to see if the current position occupied by a unit of a friendly faction
##
...
#
# Updates the astargrid to reflect the appropriate restarints for the specified combatUnit
#
func update_astar_points(combatUnit: CombatUnit):
	...
	## Update for movement classess
		if entry.blocks_unit_movement:
			if combatUnit.unit.movement_class in entry.blocks_unit_movement:
				_astargrid.set_point_solid(tile)
...
##REVIEW THIS METHOD IT SEEMS OUTDATED? LEGACY CODE
#
# Finds path to the target tile
#
func find_path(tile_position: Vector2i, current_position : Vector2i = Vector2i(0,0)) -> PackedVector2Array:
	var _path : PackedVector2Array
	#is the target tile in bounds
...
#
# Returns Terrain given a map position
#
...
#
# Returns terrain accounting for entities impacting the terrain calculation, ex. Doors and drawbridges
#
...
#
# returns a Array[Vector2i] containing the tile indexes to make a path
#
