...
func _ready():
	...
	iventory_array.clear()
	iventory_array.insert(0, ItemDatabase.items["iron_sword"])
	add_combatant(create_combatant_unit(Unit.create_generic(UnitTypeDatabase.unit_types["drake"], iventory_array, "Braumulus", 12,2, false), 1, Constants.UNIT_AI_TYPE.DEFAULT), Vector2i(12,2))
	iventory_array.clear()
	...
	iventory_array.clear()
	iventory_array.insert(0, ItemDatabase.items["iron_lance"])
	add_combatant(create_combatant_unit(Unit.create_generic(UnitTypeDatabase.unit_types["cavalier_lance"], iventory_array, "Fiend", 6,8, false), 1, Constants.UNIT_AI_TYPE.DEFAULT), Vector2i(15,21))
	iventory_array.clear()
